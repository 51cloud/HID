        ...
        while isOpened:
            ...
            if frame is not None:
                ...
                for face in faces:
                    ...
                    frame_image = Image.fromarray(src_frame)  # 'Tensor' object has no attribute '__array_interface__'
...
class ResNet(nn.Module):
    ...
    def forward(self, x, videos, tymode, bboxes=None):
        ...
        if tymode == 'train':
            emotion_maps, gaze_maps, attention_maps, scene_maps = getGaze(videos).__next__()
            b, c, t, h, w = x[0].shape
            if len(emotion_maps) == 0:
                emotion_maps = torch.zeros(b, c, t, h, w).cuda()
            else:
                emotion_maps = torch.tensor(emotion_maps).permute(0, 2, 1, 3, 4).cuda()
            if len(gaze_maps) == 0:
                gaze_maps = torch.zeros(b, c, t, h, w).cuda()
            else:
                gaze_maps = torch.tensor(gaze_maps).permute(0, 2, 1, 3, 4).cuda()
            if len(attention_maps) == 0:
                attention_maps = torch.zeros(b, c, t, h, w).cuda()
            else:
                attention_maps = torch.tensor(attention_maps).permute(0, 2, 1, 3, 4).cuda()
            if len(scene_maps) == 0:
                scene_maps = torch.zeros(b, c, t, h, w).cuda()
            else:
                scene_maps = torch.tensor(scene_maps).permute(0, 2, 1, 3, 4).cuda()
            # cl, cl_edge = build_graph(x, emotion_maps, gaze_maps, attention_maps, scene_maps)
            x = x[0].cuda()
            # x = x + 0.01 * emotion_maps + 0.01 * gaze_maps + 0.01 * attention_maps + 0.01 * scene_maps
            x = x + attention_maps + emotion_maps
            x = [x, ]
        # feature = x
