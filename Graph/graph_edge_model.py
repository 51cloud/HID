...
class CrossAttn(nn.Module):
    ...
    def forward(self, y, x):
        ...
        # print('13:', torch.cuda.memory_allocated(), torch.cuda.memory_reserved())
...
class GEM(nn.Module):
    def __init__(self, in_channels, num_classes):
        ...
        self.bn = nn.BatchNorm3d(self.num_classes * self.num_classes)
    ...
    def forward(self, class_feature, global_feature):
        ...
        # global_feature:B, T, D, C
        global_feature = global_feature.repeat(1, N, 1, 1).view(B, N, T, D, C)  # x.repeat(a,b,c) :将张量x在列的方向上重复c次，在行的方向上重复b次，在深度的方向上重复a次。
        feat = self.FAM(class_feature, global_feature)
        feat_end = feat.repeat(1, 1, 1, N, 1).view(B, -1, T, D, C)
        feat_start = feat.repeat(1, N, 1, 1, 1).view(B, -1, T, D, C)
        feat = self.ARM(feat_start, feat_end)
        edge = self.bn(self.edge_proj(feat))
        return edge
