...
import math
from Gaze.lib.pytorch_convolutional_rnn import convolutional_rnn
import numpy as np
...
class ModelSpatial(nn.Module):
    ...
    def forward(self, images, head, face):
        ...
        attn_weights = attn_weights.view(-1, 1, 49)  # -1表示动态调整
        attn_weights = F.softmax(attn_weights, dim=2)  # soft attention weights single-channel dim=2 对行操作
        ...
        # encoding_inout1 = self.fc_inout(encoding_inout)
...
class Change_Gaze_Size(nn.Module):
    def __init__(self):
        super(Change_Gaze_Size, self).__init__()
        self.conv1 = conv_bn_relu(1, 1024, kernel_size=3, stride=3, padding=1)
        self.conv2 = conv_bn_relu(1024, 2048, kernel_size=3, stride=2, padding=0)
...
class Change_Attention_Size(nn.Module):
    def __init__(self):
        super(Change_Attention_Size, self).__init__()
        self.conv1 = conv_bn_relu(1, 2048, kernel_size=1, stride=1, padding=0)
        # self.conv2 = conv_bn_relu(1024, 2048, kernel_size=3, stride=2, padding=0)
...
class Change_Scene_Size(nn.Module):
    def __init__(self):
        super(Change_Scene_Size, self).__init__()
        self.conv1 = conv_bn_relu(1, 2048, kernel_size=1, stride=1, padding=0)
...
class ModelSpatioTemporal(nn.Module):
    # Define a ResNet 50-ish arch
    def __init__(self, block=BottleneckConvLSTM, num_lstm_layers=1, bidirectional=False, layers_scene=[3, 4, 6, 3, 2],
