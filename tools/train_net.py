...
def train_epoch(
    ...
    for cur_iter, (inputs, videos, labels, _, meta) in enumerate(train_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            preds, _ = model(inputs1, meta["boxes"])
            ...
            preds = model(inputs1, videos, tymode)

           
        loss_fun = losses.get_loss_func(cfg.MODEL.LOSS_FUNC)(reduction="mean")

        # print(preds, labels)
        easy_loss = loss_fun(preds, labels)
        # print(easy_loss)
        ...
        if cfg.DETECTION.ENABLE:
            if cfg.NUM_GPUS > 1:
                all_loss = du.all_reduce([all_loss])[0]
            all_loss = all_loss.item()

            train_meter.iter_toc()
            # Update and log stats.
            train_meter.update_stats(None, None, None, all_loss, lr)
            # write to tensorboard format if available.
            if writer is not None:
                writer.add_scalars(
                    {"Train/loss": all_loss, "Train/lr": lr},
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None):
    ...
    for cur_iter, (inputs, videos, labels, _, meta) in enumerate(val_loader):
        if cfg.NUM_GPUS:
        ...
        if cfg.DETECTION.ENABLE:
            # Compute the predictions.
            _, _, _, preds, _, _ = model(inputs, _, _, meta["boxes"])
            ...
            if cfg.DATA.MULTI_LABEL:
                ...
                if writer is not None:
                    writer.add_scalars(
                        {"Val/Top1_err": top1_err, "Val/Top3_err": top3_err},
