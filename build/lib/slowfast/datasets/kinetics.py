...
class Kinetics(torch.utils.data.Dataset):
    ...
    def _construct_loader(self):
        ...
        with g_pathmgr.open(path_to_file, "r") as f:
            for clip_idx, path_label in enumerate(f.read().splitlines()):
                ...
                # path = '/home/zhouzhuo/scratch/HID/Stealing/stealing079.mp4'
    ...
    def __getitem__(self, index):
        ...
        for i_try in range(self._num_retries):
            ...
            text_path = '/home/zhouzhuo/project/HAR/info_results/stealing079.h5'
            # 使用示例
            data_dict = self.read_h5_file(text_path)
            # print('data_dict:', data_dict.keys())
            frame_count = 0
            gaze_feat = []
            emotion_feat = []
            attention_feat = []
            scene_feat = np.random.rand(8, 1, 64, 64)
            ...
            attention_feat = {}
            for i in range(8):
                # print('data_dict[str(i)].keys():', data_dict[str(frame_count)].keys())
                ...
                for person_id, attributes in frame_group.items():
                    ...
                    if person_id == 'scene_feat':
                        ...
                        if person_id not in body_images.keys():
                            body_images[person_id] = np.random.rand(3, 8, 224, 224)
                            gaze_feat[person_id] = np.random.rand(8, 1, 64, 64)
                            emotion_feat[person_id] = np.random.rand(8, 1, 64, 64)
                            attention_feat[person_id] = np.random.rand(8, 1, 64, 64)
