...
class ResNet(nn.Module):
    ...
    def _construct_network(self, cfg):
        ...
        self.pool = nn.AdaptiveAvgPool3d((8, 7, 7))
    ...
    def forward(self, person_infos, videos, tymode, bboxes=None):
        ...
        for person_id in person_infos.keys():
            ...
            # print('gaze_feat1.shape:', gaze_feat.shape)
            ...
            gaze_feat = torch.where(torch.isnan(gaze_feat), torch.tensor(0.0, dtype=gaze_feat.dtype, device=gaze_feat.device), gaze_feat)

            # print('4:', torch.cuda.memory_allocated(), torch.cuda.memory_reserved())  # 5033718272 9915334656

            emotion_feat = torch.where(torch.isnan(emotion_feat), torch.tensor(0.0, dtype=emotion_feat.dtype, device=emotion_feat.device), emotion_feat)

            # print('5:', torch.cuda.memory_allocated(), torch.cuda.memory_reserved())  # 5033718272 9915334656

            attention_feat = torch.where(torch.isnan(attention_feat), torch.tensor(0.0, dtype=attention_feat.dtype, device=attention_feat.device), attention_feat)

            # print('6:', torch.cuda.memory_allocated(), torch.cuda.memory_reserved())  # 5033718272 9915334656

            scene_feat = torch.where(torch.isnan(scene_feat), torch.tensor(0.0, dtype=scene_feat.dtype, device=scene_feat.device), scene_feat)

            # print('7:', torch.cuda.memory_allocated(), torch.cuda.memory_reserved())  # 5033718272 9915334656
            # print('gaze_feat1:', gaze_feat)
            # print('gaze_feat1.shape:', gaze_feat.shape)
            # print('emotion_feat1:', emotion_feat)
            # print('emotion_feat1.shape:', emotion_feat.shape)
            ...
            gaze_feat = self.conv1(gaze_feat)
            # print('gaze_feat2:', gaze_feat)
            # gaze_feat = self.conv2(gaze_feat)
            # print('gaze_feat3:', gaze_feat)
            # gaze_feat = self.conv3(gaze_feat)
            # print('gaze_feat4:', gaze_feat)
            # gaze_feat = self.conv4(gaze_feat)
            # print('gaze_feat5:', gaze_feat)
            gaze_feat = self.conv5(gaze_feat)
            gaze_feat = self.pool(gaze_feat)
            # print('gaze_feat6:', gaze_feat)
            # gaze_feat = self.bn1(gaze_feat)
            # print('gaze_feat3:', gaze_feat)
            # gaze_feat = self.relu1(gaze_feat)
            # print('gaze_feat4:', gaze_feat)

            # print('8:', torch.cuda.memory_allocated(), torch.cuda.memory_reserved())  # 5380731392 9915334656

            emotion_feat = emotion_feat.permute(0, 2, 1, 3, 4).float()
            emotion_feat = self.conv1(emotion_feat)
            # print('gaze_feat2:', gaze_feat)
            # emotion_feat = self.conv2(emotion_feat)
            # print('gaze_feat3:', gaze_feat)
            # emotion_feat = self.conv3(emotion_feat)
            # print('gaze_feat4:', gaze_feat)
            # emotion_feat = self.conv4(emotion_feat)
            # print('gaze_feat5:', gaze_feat)
            emotion_feat = self.conv5(emotion_feat)
            emotion_feat = self.pool(emotion_feat)

            # print('9:', torch.cuda.memory_allocated(), torch.cuda.memory_reserved())  # 5727744512 9915334656
            ...
            # print('gaze_feat3:', gaze_feat)
            # attention_feat = self.conv3(attention_feat)
            # print('gaze_feat4:', gaze_feat)
            # attention_feat = self.conv4(attention_feat)
            # print('gaze_feat5:', gaze_feat)
            attention_feat = self.conv5(attention_feat)
            attention_feat = self.pool(attention_feat)

            # print('10:', torch.cuda.memory_allocated(), torch.cuda.memory_reserved())  # 6074757632 9915334656

            scene_feat = scene_feat.permute(0, 2, 1, 3, 4).float()
            scene_feat = self.conv1(scene_feat)
            # print('scene_feat2:', scene_feat)
            # scene_feat = self.conv2(scene_feat)
            # print('scene_feat3:', scene_feat)
            # scene_feat = self.conv3(scene_feat)
            # print('scene_feat4:', scene_feat)
            # scene_feat = self.conv4(scene_feat)
            # print('scene_feat5:', scene_feat)
            scene_feat = self.conv5(scene_feat)
            # print('scene_feat6:', scene_feat)
            scene_feat = self.pool(scene_feat)

            # print('11:', torch.cuda.memory_allocated(), torch.cuda.memory_reserved())  # 6421770752 9915334656

            gaze_feat = torch.where(torch.isnan(gaze_feat),
                                    torch.tensor(0.0, dtype=gaze_feat.dtype, device=gaze_feat.device), gaze_feat)
            emotion_feat = torch.where(torch.isnan(emotion_feat),
                                       torch.tensor(0.0, dtype=emotion_feat.dtype, device=emotion_feat.device),
                                       emotion_feat)
            attention_feat = torch.where(torch.isnan(attention_feat),
                                         torch.tensor(0.0, dtype=attention_feat.dtype, device=attention_feat.device),
                                         attention_feat)
            scene_feat = torch.where(torch.isnan(scene_feat),
                                     torch.tensor(0.0, dtype=scene_feat.dtype, device=scene_feat.device), scene_feat)

            # print('12:', torch.cuda.memory_allocated(), torch.cuda.memory_reserved())  # 6424982016 9915334656

            # print('x:', x)  # x: 1
            # print('x.shape:', x[0].shape)  # x.shape: torch.Size([2, 2048, 8, 7, 7])
            # print('gaze_feat:', gaze_feat)
            # print('gaze_feat.shape:', gaze_feat.shape)  # gaze_feat.shape: torch.Size([2, 2048, 8, 64, 64])
            # print('emotion_feat:', emotion_feat)
            # print('emotion_feat.shape:', emotion_feat.shape)  # emotion_feat.shape: torch.Size([2, 2048, 8, 64, 64])
            # print('attention_feat:', attention_feat)
            # print('c:', attention_feat.shape)  # attention_feat.shape: torch.Size([2, 2048, 8, 64, 64])
            # print('scene_feat:', scene_feat)
            # print('scene_feat.shape:', scene_feat.shape)  # scene_feat.shape: torch.Size([2, 2048, 8, 64, 64])
            x = x[0]
            ...
            weighted_sum = torch.sum(cl.unsqueeze(-2).unsqueeze(-1).unsqueeze(-1) * torch.stack(feature_tensors, dim=1), dim=1)

            # print('15:', torch.cuda.memory_allocated(), torch.cuda.memory_reserved())  # 7039673344 9915334656
            # print('weighted_sum:', weighted_sum)
            person_feat = [weighted_sum, ]
            # print('person_feat:', person_feat)

            if self.enable_detection:
                person_feat = self.head(person_feat, bboxes)
            else:
...
def check_nan(tensor, name):
    if torch.isnan(tensor).any():
        ...
        tensor = torch.where(torch.isnan(tensor), torch.tensor(0.0, dtype=tensor.dtype, device=tensor.device),
                             tensor)
    return tensor