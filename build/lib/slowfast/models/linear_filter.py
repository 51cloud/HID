...
class LinearFilter(nn.Module):
    ...
    def forward(self, train_feat, test_feat_t, *args, **kwargs):
        ...
        test_feat_t = self.extract_classification_feat(test_feat_t, num_sequences)

        # Train filter
        filter, filter_iter, losses = self.get_filter(train_feat, *args, **kwargs)

        # Classify samples using all return filters
        test_scores = [self.classify(f, test_feat_t) for f in filter_iter]
    ...
    def get_filter(self, feat, bb, *args, **kwargs):
        ...
        weights = self.filter_initializer(feat, bb) # 3*20*1024*22*22  3*20*4
